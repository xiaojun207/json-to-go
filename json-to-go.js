...
function getComment(inputText) {
	...
	lines.forEach(
		(line) => {
			...
			var idx4 = line.indexOf("{")
			var idx5 = line.indexOf("]")
			var idx6 = line.indexOf("}")

			if (idx1 >= 0 && idx2 >= 0 && idx3 == -1 && idx4 == -1 && idx5 == -1 && idx6 == -1) {
				var value = line.substring(idx2 + 1, line.length) // 把值和注释一起作为新的值
				...
				outputText.push(line.substring(0, idx2) + ":" + value)
			}else {
...
function jsonToGo(json, typename, flatten = true)
{
	...
	let comment_str = json
	try
	{
		comment_str = getComment(comment_str)
		comment_str = stripComments(comment_str)
		console.log("comment_str",comment_str)
		comment = JSON.parse(comment_str)
	}
	catch (e)
	{
		// throw  e
		var idx = (e.message + "").indexOf("position ") + "position ".length
		var num = (e.message + "").substr(idx, e.message.length)

		return {
			go: "",
			error: e.message + " comment[" + num + "] \r\n" + comment_str.substr(num, 30)
		};
	}
	...
	catch (e)
	{
		...
		return {
			go: "",
			error: e.message + " json[" + num + "]" + json.substr(num, 30)
		};
	}
	...
	function parseScope(scope, depth = 0, scomment = {})
	{
		if (typeof scope === "object" && scope !== null)
		{
			if (Array.isArray(scope))
			{
				...
				if (sliceType == "struct") {
					...
					parseStruct(depth + 1, innerTabs, struct, omitempty, scomment[0]); // finally parse the struct !!
				}
				else if (sliceType == "slice") {
					parseScope(scope[0], depth, scomment[0])
				}
				else {
					if (flatten && depth >= 2) {
			...
			else
			{
				...
				parseStruct(depth + 1, innerTabs, scope, {}, scomment);
			}
		}
		else {
			if (flatten && depth >= 2){
	...
	function parseStruct(depth, innerTabs, scope, omitempty, comment)
	{
		if (flatten) {
			stack.push(
				depth >= 2
		...
		if (flatten && depth >= 2)
		{
			...
			for (let i in keys)
			{
				...
				appender(typename+" ");
				parent = typename
				parseScope(scope[keys[i]], depth, comment[keys[i]]);
